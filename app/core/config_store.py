...
ACCOUNT_DEFAULTS = {
    ...
    "is_suggested": False,
    "exchange": "toobit",
    # روی کدام حساب صرافی معامله شود؟
    #   futures      = حساب فیوچرز معمولی (پیش‌فرض)
    #   copy_trading = حساب کپی‌ترید (لیدر) — نیازمند کلید API از نوع کپی‌ترید
    # این فقط برچسب و اعتبارسنجی نیست: خودِ کلید API تعیین می‌کند درخواست روی
    # کدام حساب بنشیند (اندپوینت‌های سفارش هیچ پارامتر accountId ندارند)، پس
    # این گزینه برای درست‌بودن کلید و نمادها بررسی می‌شود نه برای مسیریابی.
    ...
    # ---- حد ضرر دنبال‌کننده (تریلینگ) ----
    # وقتی قیمت به اندازه‌ی trailing_activation_pct درصد به نفع پوزیشن حرکت
    # کرد، حد ضرر شروع به دنبال کردن قیمت می‌کند و همیشه به فاصله‌ی
    # trailing_distance_pct درصد پشت آن می‌ماند. حد ضرر فقط در جهت سود
    # جابه‌جا می‌شود و هرگز عقب نمی‌رود.
    "trailing_enabled": False,
    "trailing_activation_pct": 1.0,
    "trailing_distance_pct": 1.0,
    # چه کسی حد ضرر را جابه‌جا کند: bot یعنی خود ربات در هر چرخه، exchange
    # یعنی تریلینگ داخلی صرافی. پیش‌فرض bot است چون حالت exchange حد ضرر و
    # حد سود ثابت را روی صرافی پاک می‌کند (رفتار مستندشده‌ی توبیت) و این
    # تصمیمی است که باید آگاهانه گرفته شود، نه با آپدیت به کسی تحمیل شود.
    "trailing_mode": "bot",
