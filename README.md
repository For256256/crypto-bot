...
- **۴ استراتژی داخلی**: SuperTrend+EMA+RSI · MACD+EMA روند · برگشت بولینگر+RSI · روبان MA+Kijun+PSAR
  (فقط روی Toobit — به کندل تاریخی نیاز دارند؛ تبدیل این داده را ندارد)
...
> ⚠️ **محدودیت تبدیل (Tabdeal):** API این صرافی اندپوینت کندل تاریخی یا ticker
> ندارد (فقط اردربوک). بنابراین حساب‌های تبدیل فقط از طریق **وبهوک
> TradingView** (با `price`/`sl`/`tp` صریح در پیام Alert) قابل معامله‌اند؛
> استراتژی‌های داخلی روی این صرافی اجرا نمی‌شوند.
